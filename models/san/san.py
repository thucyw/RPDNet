...
from .lib.modules import Subtraction, Subtraction2, Aggregation
...
class mySAN(nn.Module):
    def __init__(self, sa_type, block, layers, kernels):
        ...
        c //= 2
        self.conv3_t, self.bn3_t = conv1x1(c * 4, c), nn.BatchNorm2d(c)
        self.layer3_t = self._make_layer(sa_type, block, c, layers[3], kernels[3])

        c //= 2
        self.conv2_t, self.bn2_t = conv1x1(c * 4, c), nn.BatchNorm2d(c)
        self.layer2_t = self._make_layer(sa_type, block, c, layers[2], kernels[2])

        c //= 2
        self.conv1_t, self.bn1_t = conv1x1(c * 4, c), nn.BatchNorm2d(c)
        self.layer1_t = self._make_layer(sa_type, block, c, layers[1], kernels[1])

        # c //= 2
        self.conv0_t, self.bn0_t = conv1x1(c * 2, c), nn.BatchNorm2d(c)
        self.layer0_t = self._make_layer(sa_type, block, c, layers[0], kernels[0])

        self.conv_out = conv1x1(c, 1)
    ...
    def forward(self, x):
        x0 = self.relu(self.bn_in(self.conv_in(x)))
        x1 = self.relu(self.bn0(self.layer0(self.conv0(self.pool(x0)))))
        x2 = self.relu(self.bn1(self.layer1(self.conv1(self.pool(x1)))))
        x3 = self.relu(self.bn2(self.layer2(self.conv2(self.pool(x2)))))
        x4 = self.relu(self.bn3(self.layer3(self.conv3(self.pool(x3)))))
        x = self.relu(self.bn3_t(self.layer3_t(self.conv3_t(torch.cat([x3, self.up(x4)], 1)))))
        x = self.relu(self.bn2_t(self.layer2_t(self.conv2_t(torch.cat([x2, self.up(x)], 1)))))
        x = self.relu(self.bn1_t(self.layer1_t(self.conv1_t(torch.cat([x1, self.up(x)], 1)))))
        x = self.relu(self.bn0_t(self.layer0_t(self.conv0_t(torch.cat([x0, self.up(x)], 1)))))
...
def san(sa_type, layers, kernels):
    model = mySAN(sa_type, Bottleneck, layers, kernels)
    return model


if __name__ == '__main__':
    net = san(sa_type=0, layers=(3, 4, 6, 8, 3), kernels=[3, 7, 7, 7, 7]).cuda().eval()
    print(net)
    y = net(torch.randn(4, 1, 224, 224).cuda())
    print(y.size())
